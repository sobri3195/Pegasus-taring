---
summary: "Use Qwen OAuth (free tier) in Pegasustaring"
read_when:
  - You want to use Qwen with Pegasustaring
...
```bash
pegasus-taring plugins enable qwen-portal-auth
```
...
```bash
pegasus-taring models set qwen-portal/coder-model
```

## Reuse Qwen Code CLI login

If you already logged in with the Qwen Code CLI, Pegasustaring will sync credentials
