---
summary: "Use Synthetic's Anthropic-compatible API in Pegasustaring"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Pegasustaring registers it as the
...
```bash
pegasus-taring onboard --auth-choice synthetic-api-key
```
...
Note: Pegasustaring's Anthropic client appends `/v1` to the base URL, so use
