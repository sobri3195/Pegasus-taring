---
summary: "Use OpenCode Zen and Go catalogs with Pegasustaring"
read_when:
...
OpenCode exposes two hosted catalogs in Pegasustaring:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Pegasustaring keeps the runtime provider ids
...
pegasus-taring onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
pegasus-taring onboard --auth-choice opencode-go
pegasus-taring onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
