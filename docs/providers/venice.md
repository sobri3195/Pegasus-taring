---
summary: "Use Venice AI privacy-focused models in Pegasustaring"
read_when:
  - You want privacy-focused inference in Pegasustaring
...
```bash
pegasus-taring onboard --auth-choice venice-api-key
```
...
```bash
pegasus-taring agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Pegasustaring shows all available Venice models. Pick based on your needs:
...
```bash
pegasus-taring models set venice/kimi-k2-5
pegasus-taring models set venice/claude-opus-4-6
```

List all available models:

```bash
pegasus-taring models list | grep venice
```
...
Pegasustaring automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
pegasus-taring agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
pegasus-taring agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
pegasus-taring agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
pegasus-taring agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
pegasus-taring agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `pegasus-taring models list` to see currently available models. Some models may be temporarily offline.
