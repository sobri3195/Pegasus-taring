---
summary: "Run Pegasustaring with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Pegasustaring against a local SGLang server
...
Pegasustaring can connect to SGLang using the `openai-completions` API.

Pegasustaring can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Pegasustaring will query:
