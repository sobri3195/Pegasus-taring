...
read_when:
  - Designing workspace memory (~/.pegasus-taring/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Pegasustaring integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.pegasus-taring/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.pegasus-taring/workspace` as canonical human-readable memory.
...
```
~/.pegasus-taring/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Pegasustaring**, but keep a separable core library.

### Why integrate into Pegasustaring?

- Pegasustaring already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `pegasus-taring memory recall "…" --k 25 --since 30d`
  - `pegasus-taring memory reflect --since 7d`
...
Pragmatic take for `~/.pegasus-taring/workspace`:
