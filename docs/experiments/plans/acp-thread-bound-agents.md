...
This plan defines how Pegasustaring should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- Pegasustaring core owns ACP control plane concerns
...
Pegasustaring should not reimplement ACP transport internals in core.
Pegasustaring should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in Pegasustaring, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under Pegasustaring state dir
...
Core (must be in Pegasustaring core):
...
- ACP event stream is normalized to Pegasustaring reply chunks
...
For the first production backend (`extensions/acpx`), Pegasustaring and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses Pegasustaring `sessionKey`)
...
- Pegasustaring consumes ndjson events from `acpx --format json --json-strict`
