---
summary: "CLI reference for `pegasus-taring qr` (generate iOS pairing QR + setup code)"
read_when:
...
```bash
pegasus-taring qr
pegasus-taring qr --setup-code-only
pegasus-taring qr --json
pegasus-taring qr --remote
pegasus-taring qr --url wss://gateway.example/ws
```
