---
summary: "CLI reference for `pegasus-taring configure` (interactive configuration prompts)"
read_when:
...
Tip: `pegasus-taring config` without a subcommand opens the same wizard. Use
`pegasus-taring config get|set|unset` for non-interactive edits.
...
```bash
pegasus-taring configure
pegasus-taring configure --section model --section channels
```
