---
summary: "CLI reference for `pegasus-taring onboard` (interactive onboarding)"
read_when:
...
```bash
pegasus-taring onboard
pegasus-taring onboard --flow quickstart
pegasus-taring onboard --flow manual
pegasus-taring onboard --mode remote --remote-url wss://gateway-host:18789
```
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `pegasus-taring gateway run`.
...
- Fastest first chat: `pegasus-taring dashboard` (Control UI, no channel setup).
