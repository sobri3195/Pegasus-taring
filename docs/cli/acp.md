...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Pegasustaring Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`pegasus-taring acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Pegasustaring gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
pegasus-taring acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
pegasus-taring acp --url wss://gateway-host:18789 --token-file ~/.pegasus-taring/gateway.token

# Attach to an existing session key
pegasus-taring acp --session agent:main:main

# Attach by label (must already exist)
pegasus-taring acp --session-label "support inbox"

# Reset the session key before the first prompt
pegasus-taring acp --session agent:main:main --reset-session
```
...
```bash
pegasus-taring acp client

# Point the spawned bridge at a remote Gateway
pegasus-taring acp client --server-args --url wss://gateway-host:18789 --token-file ~/.pegasus-taring/gateway.token

# Override the server command (default: pegasus-taring)
pegasus-taring acp client --server "node" --server-args pegasus-taring.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive an Pegasustaring Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `pegasus-taring acp` over stdio.

Example config (persisted):

```bash
pegasus-taring config set gateway.remote.url wss://gateway-host:18789
pegasus-taring config set gateway.remote.token <token>
```
...
```bash
pegasus-taring acp --session agent:main:main
pegasus-taring acp --session agent:design:main
pegasus-taring acp --session agent:qa:bug-123
```
...
Pegasustaring bot over ACP, use `acpx` with its built-in `pegasus-taring` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx pegasus-taring` at `pegasus-taring acp`.
3. Target the Pegasustaring session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Pegasustaring ACP session
acpx pegasus-taring exec "Summarize the active Pegasustaring session state."

# Persistent named session for follow-up turns
acpx pegasus-taring sessions ensure --name codex-bridge
acpx pegasus-taring -s codex-bridge --cwd /path/to/repo \
  "Ask my Pegasustaring work agent for recent context relevant to this repo."
```

If you want `acpx pegasus-taring` to target a specific Gateway and session key every
time, override the `pegasus-taring` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "pegasus-taring": {
      "command": "env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 pegasus-taring acp --url ws://127.0.0.1:18789 --token-file ~/.pegasus-taring/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Pegasustaring checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 node pegasus-taring.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Pegasustaring agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Pegasustaring ACP” to start a thread.
...
- `pegasus-taring acp client` sets `OPENCLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `pegasus-taring`).
