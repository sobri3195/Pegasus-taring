---
summary: "CLI reference for `pegasus-taring status` (diagnostics, probes, usage snapshots)"
read_when:
...
- Update info surfaces in the Overview; if an update is available, status prints a hint to run `pegasus-taring update` (see [Updating](/install/updating)).
