...
- `pegasus-taring gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `pegasus-taring doctor` reports no blocking config/service issues.
- `pegasus-taring channels status --probe` shows connected/ready channels.
...
pegasus-taring config get agents.defaults.models
```
...
pegasus-taring status
pegasus-taring channels status --probe
pegasus-taring pairing list --channel <channel> [--account <id>]
pegasus-taring config get channels
pegasus-taring logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `pegasus-taring config get gateway.auth.token` then paste into Control UI settings.                          |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `pegasus-taring devices list` then `pegasus-taring devices approve <requestId>`.                                                                        |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `pegasus-taring configure`). If you are running Pegasustaring via Podman using the dedicated `pegasus-taring` user, the config lives at `~pegasus-taring/.pegasus-taring/pegasus-taring.json`.
...
```bash
pegasus-taring cron status
pegasus-taring cron list
pegasus-taring cron runs --id <jobId> --limit 20
pegasus-taring system heartbeat last
pegasus-taring logs --follow
```
...
pegasus-taring approvals get --node <idOrNameOrIp>
pegasus-taring logs --follow
pegasus-taring status
```
...
pegasus-taring browser start --browser-profile pegasus-taring
...
```bash
pegasus-taring gateway status
pegasus-taring config get gateway.mode
pegasus-taring config get gateway.remote.url
pegasus-taring config get gateway.auth.mode
```
...
```bash
pegasus-taring config get gateway.bind
