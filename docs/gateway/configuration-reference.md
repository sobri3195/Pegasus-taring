...
description: "Complete field-by-field reference for ~/.pegasus-taring/pegasus-taring.json"
summary: "Complete reference for every Pegasustaring config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.pegasus-taring/pegasus-taring.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Pegasustaring uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.pegasus-taring/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `pegasus-taring doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `pegasus-taring doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["pegasus-taring-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-pegasus-taring",
          requireMention: false,
...
- Pegasustaring additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `pegasus-taring plugins install @pegasus-taring/mattermost`.
...
- `commands.callbackUrl` must resolve to the Pegasustaring gateway endpoint and be reachable from the Mattermost server.
...
Pegasustaring spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `pegasus-taring channels add` (or channel onboarding) while still on a single-account top-level channel config, Pegasustaring moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `pegasus-taring doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@pegasus-taring", "pegasus-taring"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@pegasus-taring"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `pegasus-taring.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Default: `~/.pegasus-taring/workspace`.
...
Optional repository root shown in the system prompt's Runtime line. If unset, Pegasustaring auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/pegasus-taring" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Pegasustaring assumes `anthropic` (deprecated).
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before Pegasustaring aborts it. Default: `900`.
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.pegasus-taring/sandboxes",
        docker: {
          image: "pegasus-taring-sandbox:bookworm-slim",
          containerPrefix: "pegasus-taring-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "pegasus-taring-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        ssh: {
          ...
          command: "ssh",
          workspaceRoot: "/tmp/pegasus-taring-sandboxes",
        ...
        browser: {
          enabled: false,
          image: "pegasus-taring-sandbox-browser:bookworm-slim",
          network: "pegasus-taring-sandbox-browser",
          cdpPort: 9222,
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that Pegasustaring materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.pegasus-taring/sandboxes`
...
In `remote` mode, host-local edits made outside Pegasustaring are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `pegasus-taring.json`.
noVNC observer access uses VNC auth by default and Pegasustaring emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `pegasus-taring-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.pegasus-taring/workspace",
        agentDir: "~/.pegasus-taring/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@pegasus-taring"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.pegasus-taring/workspace-home" },
      { id: "work", workspace: "~/.pegasus-taring/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, Pegasustaring resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.pegasus-taring/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.pegasus-taring/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.pegasus-taring/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.pegasus-taring/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Pegasustaring starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.pegasus-taring/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, Pegasustaring treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:pegasus-taring`   | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.pegasus-taring/attachments/<uuid>/` with a `.manifest.json`.
...
Pegasustaring uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.pegasus-taring/agents/<agentId>/agent/models.json`.
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Pegasustaring forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `pegasus-taring onboard --auth-choice opencode-zen` or `pegasus-taring onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `pegasus-taring onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `pegasus-taring onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `pegasus-taring onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `pegasus-taring onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `pegasus-taring onboard --auth-choice minimax-api`.
...
- Loaded from `~/.pegasus-taring/extensions`, `<workspace>/.pegasus-taring/extensions`, plus `plugins.load.paths`.
- Discovery accepts native Pegasustaring plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Pegasustaring plugin schema when available).
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; Pegasustaring applies those as sanitized agent settings, not as raw Pegasustaring config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `pegasus-taring plugins update`.
...
```bash
OPENCLAW_CONFIG_PATH=~/.pegasus-taring/a.json \
OPENCLAW_STATE_DIR=~/.pegasus-taring-a \
pegasus-taring gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.pegasus-taring-dev` + port `19001`), `--profile <name>` (uses `~/.pegasus-taring-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.pegasus-taring/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-pegasus-taring-token: <token>`.
...
```json5
{
  hooks: {
    gmail: {
      account: "pegasus-taring@gmail.com",
...
{
  canvasHost: {
    root: "~/.pegasus-taring/workspace/canvas",
    liveReload: true,
...
- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__pegasus-taring__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__pegasus-taring__/a2ui/`
...
- Also serves A2UI at `/__pegasus-taring__/a2ui/`.
...
- Hostname defaults to `pegasus-taring`. Override with `OPENCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.pegasus-taring/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `pegasus-taring dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.pegasus-taring/.env` (neither overrides existing vars).
...
- `secrets apply` targets supported `pegasus-taring.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.pegasus-taring/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/pegasus-taring-vault-resolver",
...
- Legacy OAuth imports from `~/.pegasus-taring/credentials/oauth.json`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/pegasus-taring/pegasus-taring.log",
    consoleLevel: "info",
...
- Default log file: `/tmp/pegasus-taring/pegasus-taring-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Pegasustaring.`).
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `pegasus-taring doctor --fix` can strip unknown keys).
...
```json5
// ~/.pegasus-taring/pegasus-taring.json
{
  gateway: { port: 18789 },
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `pegasus-taring.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
