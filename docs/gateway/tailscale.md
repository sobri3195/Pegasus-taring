...
Pegasustaring can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Pegasustaring requires a shared password.
...
(`tailscale-user-login`) without supplying a token/password. Pegasustaring verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Pegasustaring only treats a request as Serve when it arrives from loopback with
...
pegasus-taring gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want Pegasustaring to undo `tailscale serve`
