...
read_when:
  - Setting up Pegasustaring for the first time
...
Pegasustaring reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.pegasus-taring/pegasus-taring.json`.

If the file is missing, Pegasustaring uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `pegasus-taring onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    pegasus-taring onboard       # full onboarding flow
    pegasus-taring configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.pegasus-taring/pegasus-taring.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Pegasustaring only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`pegasus-taring doctor`, `pegasus-taring logs`, `pegasus-taring health`, `pegasus-taring status`)
- Run `pegasus-taring doctor` to see exact issues
- Run `pegasus-taring doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `pegasus-taring.json`.
    ...
    ```bash
    pegasus-taring config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
...
The Gateway watches `~/.pegasus-taring/pegasus-taring.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `pegasus-taring config set` for single keys.
    </Warning>
    ...
    ```bash
    pegasus-taring gateway call config.get --params '{}'  # capture payload.hash
    pegasus-taring gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.pegasus-taring/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    pegasus-taring gateway call config.patch --params '{
...
Pegasustaring reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.pegasus-taring/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Pegasustaring runs your login shell and imports only the missing keys:
