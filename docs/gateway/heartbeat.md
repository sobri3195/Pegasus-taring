...
- During heartbeat runs, Pegasustaring treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `pegasus-taring sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Pegasustaring skips the heartbeat run entirely (no model call).
...
```bash
pegasus-taring system event --text "Check for urgent follow-ups" --mode now
```
