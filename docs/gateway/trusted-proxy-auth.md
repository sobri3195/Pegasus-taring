...
read_when:
  - Running Pegasustaring behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Pegasustaring
...
- You run Pegasustaring behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Pegasustaring checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Pegasustaring extracts the user identity from the configured header
...
- Pegasustaring can stay on loopback HTTP behind the proxy.
...
If Pegasustaring itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://pegasus-taring.example.com
    to: http://pegasus-taring-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
pegasus-taring.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy pegasus-taring:18789 {
...
location / {
    ...
    proxy_pass http://pegasus-taring:18789;
...
`pegasus-taring security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Pegasustaring config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `pegasus-taring security audit` and review findings
