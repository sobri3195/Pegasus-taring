...
Pegasustaring’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`pegasus-taring agent`), so routing/permissions/config match your Gateway.
...
- select agents with `model: "pegasus-taring:<agentId>"`, `model: "agent:<agentId>"`, or `x-pegasus-taring-agent-id`
- use `x-pegasus-taring-session-key` for explicit session routing
