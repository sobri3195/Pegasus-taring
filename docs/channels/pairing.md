...
read_when:
  ...
  - Reviewing Pegasustaring security posture
title: "Pairing"
---

# Pairing

“Pairing” is Pegasustaring’s explicit **owner approval** step.
...
Stored under `~/.pegasus-taring/credentials/`:
...
3. On your phone, open the Pegasustaring iOS app → Settings → Gateway.
...
Stored under `~/.pegasus-taring/devices/`:
...
- The legacy `node.pair.*` API (CLI: `pegasus-taring nodes pending/approve`) is a
