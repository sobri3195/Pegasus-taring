...
```bash
pegasus-taring plugins install @pegasus-taring/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
pegasus-taring plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Pegasustaring" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `pegasus-taring pairing list nextcloud-talk`
  - `pegasus-taring pairing approve nextcloud-talk <CODE>`
