...
<Tabs>
  <Tab title="Socket Mode (default)">
    <Steps>
      ...
      <Step title="Configure Pegasustaring">
...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Pegasustaring HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `pegasus-taring pairing approve slack <code>`.
...
- The interactive callback values are Pegasustaring-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Pegasustaring falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Pegasustaring is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "Pegasustaring",
    "description": "Slack connector for Pegasustaring"
  },
  "features": {
    "bot_user": {
      "display_name": "Pegasustaring",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        ...
        "description": "Send a message to Pegasustaring",
        "should_escape": false
      }
    ]
  },
...
```bash
pegasus-taring pairing list slack
```
...
Pegasustaring supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, Pegasustaring falls back to normal delivery for remaining payloads.
