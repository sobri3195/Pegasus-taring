...
read_when:
  - Setting up Zalo Personal for Pegasustaring
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Pegasustaring.
...
- Install via CLI: `pegasus-taring plugins install @pegasus-taring/zalouser`
- Or from a source checkout: `pegasus-taring plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `pegasus-taring channels login --channel zalouser`
...
```bash
pegasus-taring directory self --channel zalouser
pegasus-taring directory peers list --channel zalouser --query "name"
pegasus-taring directory groups list --channel zalouser --query "work"
```
...
- `pegasus-taring pairing list zalouser`
- `pegasus-taring pairing approve zalouser <code>`
...
- On startup, Pegasustaring resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Pegasustaring stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Pegasustaring state. Example:
...
- Pegasustaring sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Pegasustaring sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `pegasus-taring channels logout --channel zalouser && pegasus-taring channels login --channel zalouser`
...
- The channel now runs fully in Pegasustaring without external CLI binaries.
