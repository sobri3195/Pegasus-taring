...
read_when:
  - Setting up Twitch chat integration for Pegasustaring
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Pegasustaring connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
pegasus-taring plugins install @pegasus-taring/twitch
```

Local checkout (when running from a git repo):

```bash
pegasus-taring plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "pegasus-taring", // Bot's Twitch account
