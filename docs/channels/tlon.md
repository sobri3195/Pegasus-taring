...
Tlon is a decentralized messenger built on Urbit. Pegasustaring connects to your Urbit ship and can
...
```bash
pegasus-taring plugins install @pegasus-taring/tlon
```

Local checkout (when running from a git repo):

```bash
pegasus-taring plugins install ./extensions/tlon
```
...
By default, Pegasustaring blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `pegasus-taring message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Pegasustaring replies in-thread.
