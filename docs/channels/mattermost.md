---
summary: "Mattermost bot setup and Pegasustaring config"
read_when:
...
```bash
pegasus-taring plugins install @pegasus-taring/mattermost
```

Local checkout (when running from a git repo):

```bash
pegasus-taring plugins install ./extensions/mattermost
```
...
4. Configure Pegasustaring and start the gateway.
...
Native slash commands are opt-in. When enabled, Pegasustaring registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Pegasustaring derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Pegasustaring.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Pegasustaring.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Pegasustaring, not `404`.
...
- Approve via:
  - `pegasus-taring pairing list mattermost`
  - `pegasus-taring pairing approve mattermost <CODE>`
...
Use these target formats with `pegasus-taring message send` or cron/webhooks:
...
Pegasustaring resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Pegasustaring sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Pegasustaring sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Pegasustaring derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Pegasustaring run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="pegasus-taring-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"pegasus-taring-mattermost-interactions",
...
`pegasus-taring message send` and cron/webhook deliveries.
