...
```bash
pegasus-taring plugins install @pegasus-taring/msteams
```

Local checkout (when running from a git repo):

```bash
pegasus-taring plugins install ./extensions/msteams
```

If you choose Teams during setup and a git checkout is detected,
Pegasustaring will offer the local install path automatically.
...
3. Configure Pegasustaring with those credentials.
...
- Talk to Pegasustaring via Teams DMs, group chats, or channels.
...
- On startup, Pegasustaring resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.pegasus-taring/pegasus-taring.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Pegasustaring, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `pegasus-taring-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `pegasus-taring plugins install @pegasus-taring/msteams`
   - From a local checkout: `pegasus-taring plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Pegasustaring in Teams", "full": "Pegasustaring in Teams" },
...
Pegasustaring handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Pegasustaring only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/PegasustaringShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Pegasustaring sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `pegasus-taring message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.pegasus-taring/msteams-polls.json`.
...
```bash
pegasus-taring message send --channel msteams \
...
pegasus-taring message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
pegasus-taring message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
pegasus-taring message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
pegasus-taring message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
