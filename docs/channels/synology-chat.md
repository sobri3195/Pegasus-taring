---
summary: "Synology Chat webhook setup and Pegasustaring config"
read_when:
  - Setting up Synology Chat with Pegasustaring
...
```bash
pegasus-taring plugins install ./extensions/synology-chat
```
...
1. Install and enable the Synology Chat plugin.
   - `pegasus-taring onboard` now shows Synology Chat in the same channel setup list as `pegasus-taring channels add`.
   - Non-interactive setup: `pegasus-taring channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Pegasustaring gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Pegasustaring.
   - Guided: `pegasus-taring onboard`
   - Direct: `pegasus-taring channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `pegasus-taring pairing list synology-chat`
  - `pegasus-taring pairing approve synology-chat <CODE>`
...
```bash
pegasus-taring message send --channel synology-chat --target 123456 --text "Hello from Pegasustaring"
pegasus-taring message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
