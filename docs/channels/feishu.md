...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Pegasustaring to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Pegasustaring releases, so no separate plugin install
is required.
...
```bash
pegasus-taring plugins install @pegasus-taring/feishu
```
...
If you just installed Pegasustaring, run onboarding:
...
2. Configuring app credentials in Pegasustaring
...
1. You already ran `pegasus-taring channels add` for Feishu
2. The gateway is running (`pegasus-taring gateway status`)
...
## Step 2: Configure Pegasustaring
...
Edit `~/.pegasus-taring/pegasus-taring.json`:
...
```bash
pegasus-taring pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  pegasus-taring pairing list feishu
  pegasus-taring pairing approve feishu <CODE>
  ```
...
2. Run `pegasus-taring logs --follow` and look for `chat_id`
...
2. Run `pegasus-taring logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `pegasus-taring gateway status`  | Show gateway status           |
| `pegasus-taring gateway install` | Install/start gateway service |
| `pegasus-taring gateway stop`    | Stop gateway service          |
| `pegasus-taring gateway restart` | Restart gateway service       |
| `pegasus-taring logs --follow`   | Tail gateway logs             |
...
4. Check logs: `pegasus-taring logs --follow`
...
5. Ensure the gateway is running: `pegasus-taring gateway status`
6. Check logs: `pegasus-taring logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.pegasus-taring/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.pegasus-taring/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
