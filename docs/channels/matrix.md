...
Matrix is an open, decentralized messaging protocol. Pegasustaring connects as a Matrix **user**
...
```bash
pegasus-taring plugins install @pegasus-taring/matrix
```

Local checkout (when running from a git repo):

```bash
pegasus-taring plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `pegasus-taring plugins install @pegasus-taring/matrix`
   - From a local checkout: `pegasus-taring plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Pegasustaring calls the same
     login endpoint, stores the access token in `~/.pegasus-taring/credentials/matrix/credentials.json`,
...
- On first connection, Pegasustaring requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Pegasustaring logs a warning.
...
`~/.pegasus-taring/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `pegasus-taring pairing list matrix`
  - `pegasus-taring pairing approve matrix <CODE>`
...
- On startup, Pegasustaring resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
