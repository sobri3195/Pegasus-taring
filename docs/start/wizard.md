...
CLI onboarding is the **recommended** way to set up Pegasustaring on macOS,
...
`pegasus-taring dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
pegasus-taring agents add <name>
```
...
`pegasus-taring configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.pegasus-taring/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `pegasus-taring doctor` first.
</Note>
...
Use `pegasus-taring agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.pegasus-taring/workspace-<agentId>`.
...
- CLI command reference: [`pegasus-taring onboard`](/cli/onboard)
