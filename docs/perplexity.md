...
Pegasustaring supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Pegasustaring also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `pegasus-taring configure --section web`. It stores the key in
`~/.pegasus-taring/pegasus-taring.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.pegasus-taring/.env` (or your service environment). See [Env vars](/help/faq#how-does-pegasus-taring-load-environment-variables).
