...
Pegasustaring can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere Pegasustaring can send audio; Telegram gets a round voice-note bubble.
...
Pegasustaring defaults to Microsoft (unless disabled via
...
TTS config lives under `messages.tts` in `pegasus-taring.json`.
...
- If `provider` is **unset**, Pegasustaring prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `microsoft`.
...
`~/.pegasus-taring/settings/tts.json`, override with `OPENCLAW_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, Pegasustaring retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, Pegasustaring:
...
Discord note: `/tts` is a built-in Discord command, so Pegasustaring registers
...
/tts limit 2000
/tts summary off
/tts audio Hello from Pegasustaring
```
