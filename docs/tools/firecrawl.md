...
Pegasustaring can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `pegasus-taring configure --section web` enables the bundled Firecrawl plugin automatically.
...
Pegasustaring always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
