...
- **gateway host** → `pegasus-taring` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Pegasustaring also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.pegasus-taring/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.pegasus-taring/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `pegasus-taring approvals allowlist ...`).
- `pegasus-taring security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `pegasus-taring doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.pegasus-taring/exec-approvals.json` directly.

CLI: `pegasus-taring approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, Pegasustaring does not treat
...
topics, Pegasustaring preserves the topic for the approval prompt and the post-approval follow-up.
