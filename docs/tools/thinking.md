...
- Provider notes:
  ...
  - Moonshot (`moonshot/*`) maps `/think off` to `thinking: { type: "disabled" }` and any non-`off` level to `thinking: { type: "enabled" }`. When thinking is enabled, Moonshot only accepts `tool_choice` `auto|none`; Pegasustaring normalizes incompatible values to `auto`.
...
- Pegasustaring resolves fast mode in this order:
...
- For `openai-codex/*`, fast mode applies the same low-latency profile on Codex Responses. Pegasustaring keeps one shared `/fast` toggle across both auth paths.
...
- Anthropic fast mode is API-key only. Pegasustaring skips Anthropic service-tier injection for Claude setup-token / OAuth auth and for non-Anthropic proxy base URLs.
