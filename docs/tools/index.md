---
summary: "Agent tool surface for Pegasustaring (browser, canvas, nodes, message, cron) replacing legacy `pegasus-taring-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `pegasus-taring-*` skills
title: "Tools"
---

# Tools (Pegasustaring)

Pegasustaring exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `pegasus-taring-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `pegasus-taring.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Pegasustaring logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:pegasus-taring`: all built-in Pegasustaring tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`pegasus-taring node run`).
...
Pegasustaring tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires an API key for the chosen provider (recommended: `pegasus-taring configure --section web`).
...
Control the dedicated Pegasustaring-managed browser.
...
- Omit `profile` for the safe default: isolated Pegasustaring-managed browser (`pegasus-taring`).
...
- When `profile` is omitted, uses `browser.defaultProfile` (defaults to `pegasus-taring`).
...
- Quick smoke: `pegasus-taring nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- Available when `agents.defaults.imageGenerationModel` is configured, or when Pegasustaring can infer a compatible image-generation default from your enabled providers plus available auth.
...
- When editing and `resolution` is omitted, Pegasustaring infers a draft/final resolution from the input image size.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `pegasus-taring gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, Pegasustaring uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- `sessions_spawn` supports inline file attachments for subagent runtime only (ACP rejects them). Each attachment has `name`, `content`, and optional `encoding` (`utf8` or `base64`) and `mimeType`. Files are materialized into the child workspace at `.pegasus-taring/attachments/<uuid>/` with a `.manifest.json` metadata file. The tool returns a receipt with `count`, `totalBytes`, per file `sha256`, and `relDir`. Attachment content is automatically redacted from transcript persistence.
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Pegasustaring clamps `tools.sessions.visibility` to `tree`.
