...
read_when:
  - Running Pegasustaring Gateway in WSL2 while Chrome lives on Windows
...
- Pegasustaring Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/pegasus-taring/pegasus-taring/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Pegasustaring and Chrome are on the same machine
...
- Pegasustaring points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Pegasustaring is not the problem yet.
...
Fix that before touching Pegasustaring config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Pegasustaring at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Pegasustaring to succeed
...
```bash
pegasus-taring browser open https://example.com --browser-profile remote
pegasus-taring browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `pegasus-taring browser tabs` returns the target
...
3. Pegasustaring config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Pegasustaring config or Control UI auth
