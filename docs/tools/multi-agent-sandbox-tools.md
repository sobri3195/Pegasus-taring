...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `pegasus-taring sandbox explain`.
...
Legacy `agent.*` configs are migrated by `pegasus-taring doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=pegasus-taring-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPENCLAW_STATE_DIR:-$HOME/.pegasus-taring}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
