...
Pegasustaring features that can generate provider usage or paid API calls.
...
- `pegasus-taring status --usage` and `pegasus-taring channels list` show provider **usage windows**
...
Pegasustaring can pick up credentials from:
...
- `pegasus-taring status --usage`
- `pegasus-taring models status --json`
...
`pegasus-taring models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
