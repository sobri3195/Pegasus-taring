...
This is the full reference for `pegasus-taring onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.pegasus-taring/pegasus-taring.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `pegasus-taring doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.pegasus-taring/credentials/oauth.json`; auth profiles live in `~/.pegasus-taring/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.pegasus-taring/credentials/oauth.json` (or `$OPENCLAW_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.pegasus-taring/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `pegasus-taring pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `pegasus-taring configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `pegasus-taring health`.
    - Tip: `pegasus-taring status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
- Stores it under `~/.pegasus-taring/tools/signal-cli/<version>/`.
...
Typical fields in `~/.pegasus-taring/pegasus-taring.json`:
...
`pegasus-taring agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.pegasus-taring/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.pegasus-taring/agents/<agentId>/sessions/`.
