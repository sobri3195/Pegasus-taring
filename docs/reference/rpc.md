...
Pegasustaring integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Pegasustaring owns lifecycle when `channels.signal.autoStart=true`.
...
- Pegasustaring spawns `imsg rpc` as a child process (legacy iMessage integration).
