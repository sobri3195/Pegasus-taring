---
summary: "How Pegasustaring builds prompt context and reports token usage + costs"
read_when:
...
Pegasustaring tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Pegasustaring assembles its own system prompt on every run. It includes:
...
For images, Pegasustaring downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Pegasustaring session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `pegasus-taring status --usage` and `pegasus-taring channels list` show
...
`cacheWrite`. If pricing is missing, Pegasustaring shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Pegasustaring can
...
Anthropic's 1M context window is currently beta-gated. Pegasustaring can inject the
...
Pegasustaring skips the `context-1m-*` beta header because Anthropic currently
