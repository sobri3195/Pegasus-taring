...
- With Anthropic API-key auth profiles, Pegasustaring seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Pegasustaring injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Pegasustaring exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.pegasus-taring/logs/cache-trace.jsonl" # optional
