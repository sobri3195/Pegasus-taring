...
- Plugin manifests (`pegasus-taring.plugin.json`) are required for all plugins.

## Plugin schema enforcement

- Each plugin provides a strict JSON Schema for its config (inline in the manifest).
- Plugin load flow:
  1. Resolve plugin manifest + schema (`pegasus-taring.plugin.json`).
...
- If config invalid:
  ...
  - Instruct: `pegasus-taring doctor --fix`.
- `pegasus-taring doctor --fix`:
  - Applies migrations.
...
Everything else must hard-fail with: “Config invalid. Run `pegasus-taring doctor --fix`.”
