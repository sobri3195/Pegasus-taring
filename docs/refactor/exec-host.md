...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.pegasus-taring/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.pegasus-taring/node.json` (node id + pairing token).
...
Path: `~/.pegasus-taring/exec-approvals.json`
...
- Unix socket at `~/.pegasus-taring/exec-approvals.sock` (0600).
