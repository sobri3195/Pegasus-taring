...
- Publish as `pegasus-taring/plugin-sdk` (or export from core under `pegasus-taring/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `PegasustaringPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: PegasustaringConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: PegasustaringConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: PegasustaringConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: PegasustaringConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: PegasustaringConfig): string;
  };
};
```
...
- Introduce `pegasus-taring/plugin-sdk`.
- Add `api.runtime` to `PegasustaringPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `pegasus-taringRuntime: ">=2026.2.0"`).
...
- `pegasus-taring plugins inspect` provides canonical deep introspection for any
