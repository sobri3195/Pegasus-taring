---
title: Fly.io
description: Deploy Pegasustaring on Fly.io
summary: "Step-by-step Fly.io deployment for Pegasustaring with persistent storage and HTTPS"
read_when:
  - Deploying Pegasustaring on Fly.io
  - Setting up Fly volumes, secrets, and first-run config
---

# Fly.io Deployment

**Goal:** Pegasustaring Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
fly apps create my-pegasus-taring

# Create a persistent volume (1GB is usually enough)
fly volumes create pegasus-taring_data --size 1 --region iad
```
...
```toml
app = "my-pegasus-taring"  # Your app name
primary_region = "iad"
...
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `pegasus-taring.json` where they could be accidentally exposed or logged.
...
```bash
mkdir -p /data
cat > /data/pegasus-taring.json << 'EOF'
{
  "agents": {
    "defaults": {
      "model": {
...
**Note:** With `OPENCLAW_STATE_DIR=/data`, the config path is `/data/pegasus-taring.json`.
...
Or visit `https://my-pegasus-taring.fly.dev/`
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/pegasus-taring.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/pegasus-taring.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/pegasus-taring.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/pegasus-taring.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/pegasus-taring.json"
```
...
```bash
# List current IPs
fly ips list -a my-pegasus-taring

# Release public IPs
fly ips release <public-ipv4> -a my-pegasus-taring
fly ips release <public-ipv6> -a my-pegasus-taring
...
fly ips allocate-v6 --private -a my-pegasus-taring
```
...
fly proxy 3000:3000 -a my-pegasus-taring
...
```bash
fly ssh console -a my-pegasus-taring
```
