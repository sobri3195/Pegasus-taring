...
- `/usage cost` in chats: local cost summary aggregated from Pegasustaring session logs.
- CLI: `pegasus-taring status --usage` prints a full per-provider breakdown.
- CLI: `pegasus-taring channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
