...
read_when:
  - You want to understand how Pegasustaring assembles model context
...
A **context engine** controls how Pegasustaring builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Pegasustaring ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.pegasus-taring/pegasus-taring.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Pegasustaring plugin. Install
first, then select the engine in the slot:

```bash
# Install from npm
pegasus-taring plugins install @martian-engineering/lossless-claw

# Or install from a local path (for development)
pegasus-taring plugins install -l ./my-context-engine
```
...
Every time Pegasustaring runs a model prompt, the context engine participates at
...
Pegasustaring currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Pegasustaring
...
The built-in `legacy` engine preserves Pegasustaring's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Pegasustaring uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Pegasustaring disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Pegasustaring automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `pegasus-taring/plugin-sdk/core` to use
  Pegasustaring's built-in compaction behavior.
...
Pegasustaring resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Pegasustaring's built-in summarization. Plugin engines can implement
...
- Use `pegasus-taring doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Pegasustaring
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `pegasus-taring plugins install -l ./my-engine` to link a
