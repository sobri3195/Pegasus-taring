...
```bash
pegasus-taring tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `pegasus-taring tui --deliver`
...
- If your terminal uses a light background and auto-detection is wrong, set `OPENCLAW_THEME=light` before launching `pegasus-taring tui`.
...
- Check the Gateway logs: `pegasus-taring logs --follow`.
- Confirm the agent can run: `pegasus-taring status` and `pegasus-taring models status`.
...
- No agents in picker: check `pegasus-taring agents list` and your routing config.
