...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/pegasus-taring`)
...
If the page fails to load, start the Gateway first: `pegasus-taring gateway`.
...
you revoke it with `pegasus-taring devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.pegasus-taring/pegasus-taring.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop pegasus-taring`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Pegasustaring
...
```bash
pegasus-taring gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
Pegasustaring **blocks** Control UI connections without device identity.
...
```bash
OPENCLAW_CONTROL_UI_BASE_PATH=/pegasus-taring/ pnpm ui:build
```
