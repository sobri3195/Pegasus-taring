...
- Jobs persist under `~/.pegasus-taring/cron/` so restarts don’t lose schedules.
...
- For upgrades, `pegasus-taring doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
pegasus-taring cron list
pegasus-taring cron run <job-id>
pegasus-taring cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.pegasus-taring/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `pegasus-taring cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Pegasustaring applies a
...
If `delivery` is omitted for isolated jobs, Pegasustaring defaults to `announce`.
...
- CLI equivalents: `pegasus-taring cron add --light-context ...` and `pegasus-taring cron edit --light-context`.
...
- Job store: `~/.pegasus-taring/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.pegasus-taring/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Pegasustaring classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
```json5
{
  cron: {
    ...
    store: "~/.pegasus-taring/cron/jobs.json",
...
- For removed run sessions no longer referenced by the session store, Pegasustaring archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `pegasus-taring cron runs` and adjust retention before logs become large
...
pegasus-taring cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
pegasus-taring cron edit <jobId> --agent ops
pegasus-taring cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
pegasus-taring cron run <jobId>
pegasus-taring cron run <jobId> --due
```

`cron.run` now acknowledges once the manual run is queued, not after the job finishes. Successful queue responses look like `{ ok: true, enqueued: true, runId }`. If the job is already running or `--due` finds nothing due, the response stays `{ ok: true, ran: false, reason }`. Use `pegasus-taring cron runs --id <jobId>` or the `cron.runs` gateway method to inspect the eventual finished entry.

Edit an existing job (patch fields):

```bash
pegasus-taring cron edit <jobId> \
...
```bash
pegasus-taring cron edit <jobId> --exact
```
...
```bash
pegasus-taring system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`pegasus-taring system event`](/cli/system).
...
- Pegasustaring applies exponential retry backoff for recurring jobs after consecutive errors:
