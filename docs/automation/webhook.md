...
read_when:
  ...
  - Wiring external systems into Pegasustaring
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Pegasustaring config directory (typically `~/.pegasus-taring/hooks/transforms`).
...
- `pegasus-taring webhooks gmail setup` writes `hooks.gmail` config for `pegasus-taring webhooks gmail run`.
...
curl -X POST http://127.0.0.1:18789/hooks/agent \
  -H 'x-pegasus-taring-token: SECRET' \
  -H 'Content-Type: application/json' \
