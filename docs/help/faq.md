---
summary: "Frequently asked questions about Pegasustaring setup, configuration, and usage"
read_when:
...
- [Quick start and first-run setup]
  ...
  - [What's the recommended way to install and set up Pegasustaring?](#whats-the-recommended-way-to-install-and-set-up-pegasus-taring)
  ...
  - [I can't access docs.pegasus-taring.ai (SSL error). What now?](#i-cant-access-docspegasus-taringai-ssl-error-what-now)
  - [What's the difference between stable and beta?](#whats-the-difference-between-stable-and-beta)
  - [How do I install the beta version, and what's the difference between beta and dev?](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  - [How do I try the latest bits?](#how-do-i-try-the-latest-bits)
  - [How long does install and onboarding usually take?](#how-long-does-install-and-onboarding-usually-take)
  - [Installer stuck? How do I get more feedback?](#installer-stuck-how-do-i-get-more-feedback)
  - [Windows install says git not found or pegasus-taring not recognized](#windows-install-says-git-not-found-or-pegasus-taring-not-recognized)
  ...
  - [How do I install Pegasustaring on Linux?](#how-do-i-install-pegasus-taring-on-linux)
  - [How do I install Pegasustaring on a VPS?](#how-do-i-install-pegasus-taring-on-a-vps)
  - [Where are the cloud/VPS install guides?](#where-are-the-cloudvps-install-guides)
  - [Can I ask Pegasustaring to update itself?](#can-i-ask-pegasus-taring-to-update-itself)
  ...
  - [If I buy a Mac mini to run Pegasustaring, can I connect it to my MacBook Pro?](#if-i-buy-a-mac-mini-to-run-pegasus-taring-can-i-connect-it-to-my-macbook-pro)
  - [Can I use Bun?](#can-i-use-bun)
  - [Telegram: what goes in `allowFrom`?](#telegram-what-goes-in-allowfrom)
  - [Can multiple people use one WhatsApp number with different Pegasustaring instances?](#can-multiple-people-use-one-whatsapp-number-with-different-pegasus-taring-instances)
  - [Can I run a "fast chat" agent and an "Opus for coding" agent?](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Does Homebrew work on Linux?](#does-homebrew-work-on-linux)
  - [What's the difference between the hackable (git) install and npm install?](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  - [Can I switch between npm and git installs later?](#can-i-switch-between-npm-and-git-installs-later)
  - [Should I run the Gateway on my laptop or a VPS?](#should-i-run-the-gateway-on-my-laptop-or-a-vps)
  - [How important is it to run Pegasustaring on a dedicated machine?](#how-important-is-it-to-run-pegasus-taring-on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run Pegasustaring in a VM and what are the requirements](#can-i-run-pegasus-taring-in-a-vm-and-what-are-the-requirements)
- [What is Pegasustaring?](#what-is-pegasus-taring)
  - [What is Pegasustaring, in one paragraph?](#what-is-pegasus-taring-in-one-paragraph)
  - [What's the value proposition?](#whats-the-value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for Pegasustaring](#what-are-the-top-five-everyday-use-cases-for-pegasus-taring)
  - [Can Pegasustaring help with lead gen outreach ads and blogs for a SaaS](#can-pegasus-taring-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills and automation](#skills-and-automation)
  ...
  - [Can Pegasustaring run tasks on a schedule or continuously in the background?](#can-pegasus-taring-run-tasks-on-a-schedule-or-continuously-in-the-background)
  - [Can I run Apple macOS-only skills from Linux?](#can-i-run-apple-macos-only-skills-from-linux)
  - [Do you have a Notion or HeyGen integration?](#do-you-have-a-notion-or-heygen-integration)
  - [How do I use my existing signed-in Chrome with Pegasustaring?](#how-do-i-use-my-existing-signed-in-chrome-with-pegasus-taring)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with Pegasustaring saved locally?](#is-all-data-used-with-pegasus-taring-saved-locally)
  - [Where does Pegasustaring store its data?](#where-does-pegasus-taring-store-its-data)
  - [Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [What's the recommended backup strategy?](#whats-the-recommended-backup-strategy)
  - [How do I completely uninstall Pegasustaring?](#how-do-i-completely-uninstall-pegasus-taring)
...
- [Config basics](#config-basics)
  ...
  - [Can the Pegasustaring browser run headless?](#can-the-pegasus-taring-browser-run-headless)
...
- [Remote gateways and nodes](#remote-gateways-and-nodes)
  ...
  - [Can two Pegasustaring instances talk to each other (local + VPS)?](#can-two-pegasus-taring-instances-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does Pegasustaring load environment variables?](#how-does-pegasus-taring-load-environment-variables)
...
- [Sessions and multiple chats](#sessions-and-multiple-chats)
  ...
  - [Is there a way to make a team of Pegasustaring instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-pegasus-taring-instances-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset Pegasustaring but keep it installed?](#how-do-i-completely-reset-pegasus-taring-but-keep-it-installed)
  ...
  - [Why doesn't Pegasustaring reply in a group?](#why-doesnt-pegasus-taring-reply-in-a-group)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do Pegasustaring, Flawd, and Krill use for models?](#what-do-pegasus-taring-flawd-and-krill-use-for-models)
...
- [Gateway: ports, "already running", and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [Why does `pegasus-taring gateway status` say `Runtime: running` but `RPC probe: failed`?](#why-does-pegasus-taring-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [Why does `pegasus-taring gateway status` show `Config (cli)` and `Config (service)` different?](#why-does-pegasus-taring-gateway-status-show-config-cli-and-config-service-different)
  - [What does "another gateway instance is already listening" mean?](#what-does-another-gateway-instance-is-already-listening-mean)
  - [How do I run Pegasustaring in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run-pegasus-taring-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart Pegasustaring?](#i-closed-my-terminal-on-windows-how-do-i-restart-pegasus-taring)
  ...
  - [ELI5: `pegasus-taring gateway restart` vs `pegasus-taring gateway`](#eli5-pegasus-taring-gateway-restart-vs-pegasus-taring-gateway)
  - [What's the fastest way to get more details when something fails?](#whats-the-fastest-way-to-get-more-details-when-something-fails)
- [Media and attachments](#media-and-attachments)
  - [My skill generated an image/PDF, but nothing was sent](#my-skill-generated-an-imagepdf-but-nothing-was-sent)
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose Pegasustaring to inbound DMs?](#is-it-safe-to-expose-pegasus-taring-to-inbound-dms)
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/pegasus-taring/pegasus-taring-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   pegasus-taring health --verbose   # shows the target URL + config path on errors
   ```
...
```bash
curl -fsSL https://pegasus-taring.ai/install.sh | bash -s -- --install-method git
```

This installs Pegasustaring **from a git checkout**, so the agent can read the code + docs and
...
[https://github.com/pegasus-taring/pegasus-taring/issues](https://github.com/pegasus-taring/pegasus-taring/issues)
[https://github.com/pegasus-taring/pegasus-taring/pulls](https://github.com/pegasus-taring/pegasus-taring/pulls)

Start with these commands (share outputs when asking for help):

```bash
pegasus-taring status
pegasus-taring models status
pegasus-taring doctor
```

What they do:

- `pegasus-taring status`: quick snapshot of gateway/agent health + basic config.
- `pegasus-taring models status`: checks provider auth + model availability.
- `pegasus-taring doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `pegasus-taring status --all`, `pegasus-taring logs --follow`,
`pegasus-taring gateway status`, `pegasus-taring health --verbose`.

Quick debug loop: [First 60 seconds if something's broken](#first-60-seconds-if-somethings-broken).
Install docs: [Install](/install), [Installer flags](/install/installer), [Updating](/install/updating).

### What's the recommended way to install and set up Pegasustaring

The repo recommends running from source and using onboarding:

```bash
curl -fsSL https://pegasus-taring.ai/install.sh | bash
pegasus-taring onboard --install-daemon
```
...
```bash
git clone https://github.com/pegasus-taring/pegasus-taring.git
cd pegasus-taring
pnpm install
pnpm build
...
If you don't have a global install yet, run it via `pnpm pegasus-taring onboard`.
...
- Retrieve it from the gateway host: `pegasus-taring config get gateway.auth.token` (or generate one: `pegasus-taring doctor --generate-gateway-token`).

**Not on localhost:**

- **Tailscale Serve** (recommended): keep bind loopback, run `pegasus-taring gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no token, assumes trusted gateway host); HTTP APIs still require token/password.
- **Tailnet bind**: run `pegasus-taring gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
...
1. Install Pegasustaring on the new machine.
2. Copy `$OPENCLAW_STATE_DIR` (default: `~/.pegasus-taring`) from the old machine.
3. Copy your workspace (default: `~/.pegasus-taring/workspace`).
4. Run `pegasus-taring doctor` and restart the Gateway service.
...
under `~/.pegasus-taring/` (for example `~/.pegasus-taring/agents/<agentId>/sessions/`).

Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-pegasus-taring-store-its-data),
...
[https://github.com/pegasus-taring/pegasus-taring/blob/main/CHANGELOG.md](https://github.com/pegasus-taring/pegasus-taring/blob/main/CHANGELOG.md)

Newest entries are at the top. If the top section is marked **Unreleased**, the next dated
section is the latest shipped version. Entries are grouped by **Highlights**, **Changes**, and
**Fixes** (plus docs/other sections when needed).

### I can't access docs.pegasus-taring.ai SSL error What now

Some Comcast/Xfinity connections incorrectly block `docs.pegasus-taring.ai` via Xfinity
Advanced Security. Disable it or allowlist `docs.pegasus-taring.ai`, then retry. More
detail: [Troubleshooting](/help/troubleshooting#docspegasus-taringai-shows-an-ssl-error-comcastxfinity).
Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

If you still can't reach the site, the docs are mirrored on GitHub:
[https://github.com/pegasus-taring/pegasus-taring/tree/main/docs](https://github.com/pegasus-taring/pegasus-taring/tree/main/docs)
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://pegasus-taring.ai/install.sh | bash -s -- --beta
```

```bash
curl -fsSL --proto '=https' --tlsv1.2 https://pegasus-taring.ai/install.sh | bash -s -- --install-method git
```

Windows installer (PowerShell):
[https://pegasus-taring.ai/install.ps1](https://pegasus-taring.ai/install.ps1)
...
```bash
pegasus-taring update --channel dev
```
...
```bash
curl -fsSL https://pegasus-taring.ai/install.sh | bash -s -- --verbose
```

Beta install with verbose:

```bash
curl -fsSL https://pegasus-taring.ai/install.sh | bash -s -- --beta --verbose
```

For a hackable (git) install:

```bash
curl -fsSL https://pegasus-taring.ai/install.sh | bash -s -- --install-method git --verbose
```
...
Set-PSDebug -Trace 1
& ([scriptblock]::Create((iwr -useb https://pegasus-taring.ai/install.ps1))) -NoOnboard
Set-PSDebug -Trace 0
```

More options: [Installer flags](/install/installer).

### Windows install says git not found or pegasus-taring not recognized
...
**2) pegasus-taring is not recognized after install**
...
If you still reproduce this on latest Pegasustaring, track/report it in:

- [Issue #30640](https://github.com/pegasus-taring/pegasus-taring/issues/30640)
...
### How do I install Pegasustaring on Linux
...
### How do I install Pegasustaring on a VPS
...
### Can I ask Pegasustaring to update itself
...
```bash
pegasus-taring update
pegasus-taring update status
pegasus-taring update --channel stable|beta|dev
pegasus-taring update --tag <dist-tag|version>
pegasus-taring update --no-restart
```

If you must automate from an agent:

```bash
pegasus-taring update --yes --no-restart
pegasus-taring gateway restart
```
...
`pegasus-taring onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run Pegasustaring with **API keys** (Anthropic/OpenAI/others) or with
...
OpenAI Codex OAuth is explicitly supported for external tools like Pegasustaring.
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in onboarding or paste it with `pegasus-taring models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in onboarding. If you want to run it on the gateway host, use `pegasus-taring models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `pegasus-taring models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth (Claude Pro or Max)

Yes - via **setup-token**. Pegasustaring no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so Pegasustaring can keep replying while a provider is rate-limited.
...
Pegasustaring supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). Onboarding can run the OAuth flow and will set the default model to `openai-codex/gpt-5.4` when appropriate. See [Model providers](/concepts/model-providers) and [Onboarding (CLI)](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. Pegasustaring fully supports **OpenAI Code (Codex) subscription OAuth**.
OpenAI explicitly allows subscription OAuth usage in external tools/workflows
like Pegasustaring. Onboarding can run the OAuth flow for you.

See [OAuth](/concepts/oauth), [Model providers](/concepts/model-providers), and [Onboarding (CLI)](/start/wizard).

### How do I set up Gemini CLI OAuth

Gemini CLI uses a **plugin auth flow**, not a client id or secret in `pegasus-taring.json`.

Steps:

1. Enable the plugin: `pegasus-taring plugins enable google`
2. Login: `pegasus-taring models auth login --provider google-gemini-cli --set-default`

This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/concepts/model-providers).

### Is a local model OK for casual chats

Usually no. Pegasustaring needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.5 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. Pegasustaring runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
### If I buy a Mac mini to run Pegasustaring can I connect it to my MacBook Pro
...
- Use `pegasus-taring nodes status` / `pegasus-taring nodes list` to see it.
...
Onboarding accepts `@username` input and resolves it to a numeric ID, but Pegasustaring authorization uses numeric IDs only.

Safer (no third-party bot):

- DM your bot, then run `pegasus-taring logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different Pegasustaring instances
...
If you run Pegasustaring via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
...
This **does not delete your data** - it only changes the Pegasustaring code install. Your state
(`~/.pegasus-taring`) and workspace (`~/.pegasus-taring/workspace`) stay untouched.
...
```bash
npm install -g pegasus-taring@latest
pegasus-taring doctor
pegasus-taring gateway restart
```
...
**Pegasustaring-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run Pegasustaring on a dedicated machine
...
Pegasustaring is lightweight. For a basic Gateway + one chat channel:
...
### Can I run Pegasustaring in a VM and what are the requirements
...
## What is Pegasustaring?

### What is Pegasustaring in one paragraph

Pegasustaring is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### What's the value proposition

Pegasustaring is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for Pegasustaring
...
### Can Pegasustaring help with lead gen outreach ads and blogs for a SaaS
...
Pegasustaring draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

Pegasustaring is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use Pegasustaring when you
...
Showcase: [https://pegasus-taring.ai/showcase](https://pegasus-taring.ai/showcase)

## Skills and automation

### How do I customize skills without keeping the repo dirty

Use managed overrides instead of editing the repo copy. Put your changes in `~/.pegasus-taring/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.pegasus-taring/pegasus-taring.json`). Precedence is `<workspace>/skills` > `~/.pegasus-taring/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.pegasus-taring/pegasus-taring.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.pegasus-taring/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Pegasustaring treats as `<workspace>/skills`.
...
```bash
pegasus-taring cron run <jobId> --force
pegasus-taring cron runs --id <jobId> --limit 50
```
...
### Can Pegasustaring run tasks on a schedule or continuously in the background
...
Not directly. macOS skills are gated by `metadata.pegasus-taring.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.

You have three supported patterns:

**Option A - run the Gateway on a Mac (simplest).**
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-pegasus-taring-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.

**Option B - use a macOS node (no SSH).**
Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. Pegasustaring can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
...
3. Override the skill metadata (workspace or `~/.pegasus-taring/skills`) to allow Linux:

   ```markdown
   ---
   name: apple-notes
   description: Manage Apple Notes via the memo CLI on macOS.
   metadata: { "pegasus-taring": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
   ---
   ```
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured Pegasustaring workspace); Pegasustaring treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.pegasus-taring/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).

### How do I use my existing signed-in Chrome with Pegasustaring
...
pegasus-taring browser --browser-profile user snapshot
```
...
Pegasustaring memory is just Markdown files in the agent workspace:

- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

Pegasustaring also runs a **silent pre-compaction memory flush** to remind the model
...
If you don't set a provider explicitly, Pegasustaring auto-selects a provider when it
...
configured and present, Pegasustaring
...
### Is all data used with Pegasustaring saved locally

No - **Pegasustaring's state is local**, but **external services still see what you send them**.

- **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
  (`~/.pegasus-taring` + your workspace directory).
...
### Where does Pegasustaring store its data

Everything lives under `$OPENCLAW_STATE_DIR` (default: `~/.pegasus-taring`):

| Path                                                            | Purpose                                                            |
| --------------------------------------------------------------- | ------------------------------------------------------------------ |
| `$OPENCLAW_STATE_DIR/pegasus-taring.json`                             | Main config (JSON5)                                                |
...
Legacy single-agent path: `~/.pegasus-taring/agent/*` (migrated by `pegasus-taring doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.pegasus-taring/workspace`).

### Where should AGENTSmd SOULmd USERmd MEMORYmd live

These files live in the **agent workspace**, not `~/.pegasus-taring`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or legacy fallback `memory.md` when `MEMORY.md` is absent),
  `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.pegasus-taring`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.pegasus-taring/skills`).

Default workspace is `~/.pegasus-taring/workspace`, configurable via:

```json5
{
  agents: { defaults: { workspace: "~/.pegasus-taring/workspace" } },
}
```
...
Do **not** commit anything under `~/.pegasus-taring` (credentials, sessions, tokens, or encrypted secrets payloads).
If you need a full restore, back up both the workspace and the state directory
separately (see the migration question above).

Docs: [Agent workspace](/concepts/agent-workspace).

### How do I completely uninstall Pegasustaring
...
`workspace` to the repo root. The Pegasustaring repo is just source code; keep the
...
Pegasustaring reads an optional **JSON5** config from `$OPENCLAW_CONFIG_PATH` (default: `~/.pegasus-taring/pegasus-taring.json`):

```
$OPENCLAW_CONFIG_PATH
```

If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.pegasus-taring/workspace`).
...
Pegasustaring enforces token auth by default, including loopback. If no token is configured, gateway startup auto-generates one and saves it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `pegasus-taring doctor --generate-gateway-token`.
...
- `default`: uses `All your chats, one Pegasustaring.` every time.
...
**Recommended:** run `pegasus-taring configure --section web` and choose a provider.
...
- Daemons read env vars from `~/.pegasus-taring/.env` (or the service environment).
...
### Can the Pegasustaring browser run headless
...
- Gateway is running: `pegasus-taring gateway status`
- Gateway health: `pegasus-taring status`
- Channel health: `pegasus-taring channels status`
...
### Can two Pegasustaring instances talk to each other local VPS
...
`pegasus-taring agent --message ... --deliver`, targeting a chat where the other bot
...
```bash
pegasus-taring agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
```
...
- Restore from backup (git or a copied `~/.pegasus-taring/pegasus-taring.json`).
- If you have no backup, re-run `pegasus-taring doctor` and reconfigure channels/models.
- If this was unexpected, file a bug and include your last known config or any backup.
- A local coding agent can often reconstruct a working config from logs or history.

Avoid it:

- Use `pegasus-taring config set` for small changes.
- Use `pegasus-taring configure` for interactive edits.
...
```bash
pegasus-taring gateway --tailscale serve
```
...
### How does Pegasustaring load environment variables

Pegasustaring reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

- `.env` from the current working directory
- a global fallback `.env` from `~/.pegasus-taring/.env` (aka `$OPENCLAW_STATE_DIR/.env`)
...
1. Put the missing keys in `~/.pegasus-taring/.env` so they're picked up even when the service doesn't inherit your shell env.
...
`pegasus-taring models status` reports whether **shell env import** is enabled. "Shell env: off"
does **not** mean your env vars are missing - it just means Pegasustaring won't load
your login shell automatically.

If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
environment. Fix by doing one of these:

1. Put the token in `~/.pegasus-taring/.env`:
...
### Is there a way to make a team of Pegasustaring instances one CEO and many agents
...
### How do I completely reset Pegasustaring but keep it installed
...
```bash
pegasus-taring reset --scope full --yes --non-interactive
```
...
- If you used profiles (`--profile` / `OPENCLAW_PROFILE`), reset each state dir (defaults are `~/.pegasus-taring-<profile>`).
- Dev reset: `pegasus-taring gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
headers like `# Heading`), Pegasustaring skips the heartbeat run to save API calls.
...
No. Pegasustaring runs on **your own account**, so if you're in the group, Pegasustaring can see it.
...
```bash
pegasus-taring logs --follow --json
```
...
```bash
pegasus-taring directory groups list --channel whatsapp
```

Docs: [WhatsApp](/channels/whatsapp), [Directory](/cli/directory), [Logs](/cli/logs).

### Why doesn't Pegasustaring reply in a group
...
- **Disk growth:** sessions + transcripts live under `~/.pegasus-taring/agents/<agentId>/sessions/`.
...
- Use `pegasus-taring doctor` to spot stray workspaces and profile mismatches.
...
Pegasustaring's default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, Pegasustaring currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
4. Run `pegasus-taring onboard` and choose `Ollama`
...
- for manual switching, use `pegasus-taring models list` and `pegasus-taring models set ollama/<model>`
...
- `pegasus-taring models set ...` (updates just model config)
- `pegasus-taring configure --section model` (interactive)
- edit `agents.defaults.model` in `~/.pegasus-taring/pegasus-taring.json`

Avoid `config.apply` with a partial object unless you intend to replace the whole config.
If you did overwrite config, restore from backup or re-run `pegasus-taring doctor` to repair.

Docs: [Models](/concepts/models), [Configure](/cli/configure), [Config](/cli/config), [Doctor](/gateway/doctor).

### What do Pegasustaring, Flawd, and Krill use for models

- These deployments can differ and may change over time; there is no fixed provider recommendation.
- Check the current runtime setting on each gateway with `pegasus-taring models status`.
...
Yes. Pegasustaring ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
```
~/.pegasus-taring/agents/<agentId>/agent/auth-profiles.json
```

Fix options:

- Run `pegasus-taring agents add <id>` and configure auth during the wizard.
...
Cooldowns apply to failing profiles (exponential backoff), so Pegasustaring can keep responding even when a provider is rate-limited or temporarily failing.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/.pegasus-taring/agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/.pegasus-taring/agent/*` (migrated by `pegasus-taring doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.pegasus-taring/.env` or enable `env.shellEnv`.
- **Make sure you're editing the correct agent**
  - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
- **Sanity-check model/auth status**
  - Use `pegasus-taring models status` to see configured models and whether providers are authenticated.
...
- **Use a setup-token**
  - Run `claude setup-token`, then paste it with `pegasus-taring models auth setup-token --provider anthropic`.
  - If the token was created on another machine, use `pegasus-taring models auth paste-token --provider anthropic`.
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.pegasus-taring/.env` on the **gateway host**.
  - Clear any pinned order that forces a missing profile:

    ```bash
    pegasus-taring models auth order clear --provider anthropic
    ```
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), Pegasustaring will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
...
Fix: Pegasustaring now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
Pegasustaring uses provider-prefixed IDs like:
...
Pegasustaring may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `pegasus-taring models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.

You can also set a **per-agent** order override (stored in that agent's `auth-profiles.json`) via the CLI:

```bash
# Defaults to the configured default agent (omit --agent)
pegasus-taring models auth order get --provider anthropic

# Lock rotation to a single profile (only try this one)
pegasus-taring models auth order set --provider anthropic anthropic:default

# Or set an explicit order (fallback within provider)
pegasus-taring models auth order set --provider anthropic anthropic:work anthropic:default
...
```bash
pegasus-taring models auth order set --provider anthropic --agent main anthropic:default
```
...
### Why does pegasus-taring gateway status say Runtime running but RPC probe failed

Because "running" is the **supervisor's** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

Use `pegasus-taring gateway status` and trust these lines:

- `Probe target:` (the URL the probe actually used)
- `Listening:` (what's actually bound on the port)
- `Last gateway error:` (common root cause when the process is alive but the port isn't listening)

### Why does pegasus-taring gateway status show Config cli and Config service different
...
```bash
pegasus-taring gateway install --force
```
...
Pegasustaring enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `pegasus-taring gateway --port <port>`.

### How do I run Pegasustaring in remote mode client connects to a Gateway elsewhere
...
- `pegasus-taring gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
- Fastest: `pegasus-taring dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
- If you don't have a token yet: `pegasus-taring doctor --generate-gateway-token`.
...
- If mismatch persists after the one retry, rotate/re-approve the paired device token:
  - `pegasus-taring devices list`
  - `pegasus-taring devices rotate --device <id> --role operator`
- Still stuck? Run `pegasus-taring status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
- Use `pegasus-taring --profile <name> …` per instance (auto-creates `~/.pegasus-taring-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `pegasus-taring --profile <name> gateway install`.

Profiles also suffix service names (`ai.pegasus-taring.<profile>`; legacy `com.pegasus-taring.*`, `pegasus-taring-gateway-<profile>.service`, `Pegasustaring Gateway (<profile>)`).
...
```
pegasus-taring tui --url ws://<host>:18789 --token <token>
```
...
```
/tmp/pegasus-taring/pegasus-taring-YYYY-MM-DD.log
```
...
- macOS: `$OPENCLAW_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.pegasus-taring/logs/...`; profiles use `~/.pegasus-taring-<profile>/logs/...`)
- Linux: `journalctl --user -u pegasus-taring-gateway[-<profile>].service -n 200 --no-pager`
- Windows: `schtasks /Query /TN "Pegasustaring Gateway (<profile>)" /V /FO LIST`
...
If you run the gateway manually, `pegasus-taring gateway --force` can reclaim the port. See [Gateway](/gateway).

### I closed my terminal on Windows how do I restart Pegasustaring
...
pegasus-taring status
pegasus-taring models status
pegasus-taring channels status
pegasus-taring logs --follow
```
...
1. Is the Gateway running? `pegasus-taring gateway status`
2. Is the Gateway healthy? `pegasus-taring status`
3. Does the UI have the right token? `pegasus-taring dashboard`
...
```bash
pegasus-taring channels status
pegasus-taring channels logs --channel telegram
```

Then match the error:

- `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. Pegasustaring already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
...
### ELI5 pegasus-taring gateway restart vs pegasus-taring gateway

- `pegasus-taring gateway restart`: restarts the **background service** (launchd/systemd).
- `pegasus-taring gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use `pegasus-taring gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Pegasustaring assistant setup](/start/pegasus-taring) and [Agent send](/tools/agent-send).

CLI sending:

```bash
pegasus-taring message send --target +15555550123 --message "Here you go" --media /path/to/file.png
```
...
### Is it safe to expose Pegasustaring to inbound DMs

Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

- Default behavior on DM-capable channels is **pairing**:
  - Unknown senders receive a pairing code; the bot does not process their message.
  - Approve with: `pegasus-taring pairing approve --channel <channel> [--account <id>] <code>`
  - Pending requests are capped at **3 per channel**; check `pegasus-taring pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
- Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

Run `pegasus-taring doctor` to surface risky DM policies.
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. Pegasustaring only replies to chats it receives or to explicit sends you trigger.

Approve pairing with:

```bash
pegasus-taring pairing approve whatsapp <code>
```

List pending requests:

```bash
pegasus-taring pairing list whatsapp
```
...
Pegasustaring blocks **cross-provider** messaging by default. If a tool call is bound
