...
- Same path: run the app from a fixed location (for Pegasustaring, `dist/Pegasustaring.app`).
...
```bash
sudo tccutil reset Accessibility ai.pegasus-taring.mac
sudo tccutil reset ScreenCapture ai.pegasus-taring.mac
...
Workaround: move files into the Pegasustaring workspace (`~/.pegasus-taring/workspace`) if you want to avoid per-folder grants.
