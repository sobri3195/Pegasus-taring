...
service via the external `pegasus-taring` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.pegasus-taring.gateway`
  (or `ai.pegasus-taring.<profile>` when using `--profile`/`OPENCLAW_PROFILE`; legacy `com.pegasus-taring.*` is supported).
