---
summary: "Pegasustaring on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Pegasustaring on a Raspberry Pi
  - Running Pegasustaring on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Pegasustaring on Raspberry Pi

## Goal

Run a persistent, always-on Pegasustaring Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u pegasus-taring -f
```

## 9) Access the Pegasustaring Dashboard
...
```bash
ssh user@gateway-host 'pegasus-taring dashboard --no-open'
```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/pegasus-taring-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/pegasus-taring-compile-cache
mkdir -p /var/tmp/pegasus-taring-compile-cache
...
If this Pi is mostly running Pegasustaring, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit pegasus-taring
```

```ini
[Service]
Environment=OPENCLAW_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/pegasus-taring-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Pegasustaring state/cache on SSD-backed storage to avoid SD-card
...
Most Pegasustaring features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled pegasus-taring

# Enable if not
sudo systemctl enable pegasus-taring

# Start on boot
sudo systemctl start pegasus-taring
```
...
```bash
# Check logs
journalctl -u pegasus-taring --no-pager -n 100

# Common fix: rebuild
cd ~/pegasus-taring  # if using hackable install
npm run build
sudo systemctl restart pegasus-taring
```
