...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `pegasus-taring.internal.`), **or**
...
- Only genuine Pegasustaring iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Pegasustaring builds.
...
The Gateway advertises `_pegasus-taring-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `pegasus-taring.internal.`) and Tailscale split DNS.
...
```bash
pegasus-taring nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__pegasus-taring__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__pegasus-taring__/canvas/` and `/__pegasus-taring__/a2ui/`.
...
```bash
pegasus-taring nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__pegasus-taring; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
pegasus-taring nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `pegasus-taring devices list` and approve manually.
