...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Pegasustaring
...
Running Pegasustaring via Node with `tsx` fails at startup with:

```
[pegasus-taring] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `pegasus-taring status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch pegasus-taring.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node pegasus-taring.mjs status` works on Node 25.
