---
summary: "VPS hosting hub for Pegasustaring (Oracle/Fly/Hetzner/GCP/exe.dev)"
read_when:
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/pegasus-taring-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- Add service env for stable startup path:
  ...
  - `NODE_COMPILE_CACHE=/var/tmp/pegasus-taring-compile-cache`
