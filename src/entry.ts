...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "pegasus-taring.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "pegasus-taring.js", entryBasename: "entry.js" },
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    child.once("error", (error) => {
      console.error(
        "[pegasus-taring] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      ...
      .catch((error) => {
        console.error(
          "[pegasus-taring] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[pegasus-taring] ${parsed.error}`);
      process.exit(2);
    }
...
export function tryHandleRootHelpFastPath(
  ...
  const handleError =
    ...
    ((error: unknown) => {
      console.error(
        "[pegasus-taring] Failed to display help:",
        error instanceof Error ? (error.stack ?? error.message) : error,
      );
      process.exitCode = 1;
    });
...
function runMainOrRootHelp(argv: string[]): void {
  ...
  import("./cli/run-main.js")
    ...
    .catch((error) => {
      console.error(
        "[pegasus-taring] Failed to start CLI:",
        error instanceof Error ? (error.stack ?? error.message) : error,
      );
      process.exitCode = 1;
    });
